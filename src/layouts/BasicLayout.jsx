...
import { history as router, connect, withRouter } from 'umi'
...
function BasicLayout (props) {
  ...
  const getBreadcrumbs = () => {
    const list = _.get(activePane, 'breadcrumbs') || []
    if (list.length === 0 && _.get(activePane, 'ID') && _.get(activePane, 'ParentCode')) {
      list.push({
        LocaleKey: _.get(activePane, 'Code'),
        Name: _.get(activePane, 'Name')
      })
      const getParentsPane = (item) => {
        const data = _.get(props, 'menus', []).find(x => x.Code === _.get(item, 'ParentCode'))
        if (data && data.Code !== 'default') {
          list.unshift({
            LocaleKey: _.get(data, 'Code'),
            Name: _.get(data, 'Name')
          })
          if (_.get(data, 'ParentCode')) {
            getParentsPane(data)
          }
        }
      }
      getParentsPane(activePane)
    }
    return list
  }
