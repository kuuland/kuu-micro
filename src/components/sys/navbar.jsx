...
import { connect, withRouter, history as router } from 'umi'

import { Avatar, Menu, Dropdown, Icon, Divider, Modal, Radio, Input, Select } from 'antd'
import { get, post, del, withLocale, config, clearToken } from 'kuu-tools'
    ...
    if (e.key.startsWith(menuKeyPrefix)) {
      ...
      switch (e.key) {
        case 'profile':
          break
        case 'change_passwd':
          this.setState({ showChangePasswd: true })
          break
        case 'i18n':
                  ...
                  {users.map(item => {
                    const content = (
                      <div>
                        ...
                        >{moment.unix(item.Exp).fromNow()}
                        </span>
                      </div>
                    )
                    return (
                      <Select.Option
                        key={item.ID} title={content} value={item.ID}
                        keyword={`${item.Username}${item.Name}`}
                      >{content}
                      </Select.Option>
                    )
                  })}
  ...
  render () {
        ...
        <Dropdown
          overlay={
            <Menu onClick={this.handleMenuClick}>
              ...
              <Menu.Item key='change_passwd'>
                <Icon type='tool' />{this.props.L('kuu_navbar_changepasswd', '修改密码')}
              </Menu.Item>
    ...
    return (
      ...
      >
        ...
        <ChangePasswd
          visible={this.state.showChangePasswd}
          onCancel={() => this.setState({ showChangePasswd: false })}
        />
      </div>
    )
  }
